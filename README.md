...
- **Dietary substitution.** The same ingredient requires different substitutes depending on which step uses it. An egg in a cake batter (a `mix` step) is acting as a binder — a flax egg works. The same egg in a crème brûlée (a `steam` step) is forming a heat-set protein gel — flax egg won't set. "Replace all eggs with flax eggs" as a text substitution produces edible cake and inedible custard. Knowing which step the egg appears in is what makes the substitution decision computable rather than a guess.
