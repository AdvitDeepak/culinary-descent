...
- **Scaling and modification.** Doubling a recipe isn't just doubling every number. Cook times scale differently than quantities, and some steps change character at different scales. None of this is recoverable from text without re-parsing. (Out of scope for this checkpoint; the representation is designed to support it.)
